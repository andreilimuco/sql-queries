...
SELECT bands.name AS 'BAND NAME' FROM bands JOIN albums ON bands.id = albums.band_id GROUP BY albums.band_id HAVING COUNT(albums.id) > 0;
SELECT * FROM bands LEFT JOIN albums ON bands.id = albums.band_id;
...
SELECT albums.name AS 'NAME', albums.release_year AS 'RELEASE YEAR', SUM(songs.length) AS DURATION FROM albums JOIN songs ON albums.id = songs.album_id GROUP BY songs.album_id ORDER BY DURATION DESC LIMIT 1;

UPDATE albums SET release_year = 1986 WHERE id = 4;

SELECT * FROM albums;

INSERT INTO bands (name) VALUES ('Maroon 5');

SELECT * FROM bands;

INSERT INTO albums (name, release_year, band_id) VALUES ('Overexposed', 2012, 8);
...
SELECT bands.name AS 'band', COUNT(songs.id) AS 'Number of Songs' FROM bands JOIN albums ON bands.id = albums.band_id JOIN songs ON albums.id = songs.album_id GROUP BY albums.band_id;
...
pol_id INT NOT NULL, 
PRIMARY KEY(id),
FOREIGN KEY(pol_id) REFERENCES policies(id)
);
...
accident_descr VARCHAR(255) NOT NULL,
damage_descr VARCHAR(255) NOT NULL,
...
VALUES ('Regem', 'Martin', 'Laguna'), ('McKevin', 'Aranda', 'Cavite');

INSERT INTO policies (eff_date, exp_date, customer_id) 
VALUES ('2022-11-16', '2023-04-16', 1), ('2022-12-16', '2023-05-16', 2);

INSERT INTO policy_holders(first_name, last_name, birth_date, address, license_num, license_issued, pol_id)
VALUES ('Regem', 'Martin', '1995-01-01', 'Laguna', 'N03-12-123456', '2020-10-10', 1), ('McKevin', 'Aranda', '1997-03-12', 'Cavite', 'D01-12-123456', '2018-12-20',  2);

INSERT INTO policy_holders(first_name, last_name, birth_date, address, license_num, license_issued, pol_id)
VALUES ('Maria', 'Martin', '1968-02-24', 'Laguna', 'N03-12-123453', '1991-10-10', 1);

SELECT * FROM policy_holders;

INSERT INTO vehicles (make, model, year, type, fuel_type, purchase_price, premium_charged, pol_id) 
VALUES('Nissan', 'Almera', 2018, '4-door sedan', 'Petrol', 380000, 7000, 1), ('Toyota', 'Innova', 2016, 'MPV', 'Diesel', 1000000, 8000, 2);

SELECT * FROM customers JOIN policies ON customers.id = policies.customer_id JOIN policy_holders ON policies.id = policy_holders.pol_id;

SELECT * FROM customers AS c JOIN policies AS p ON c.id = p.customer_id JOIN vehicles as v ON p.id = v.pol_id;

SELECT * from customers where first_name = 'McKevin' and last_name = 'Aranda';

SELECT * FROM policies where customer_id = 2;

SELECT * FROM vehicles where pol_id = 2;

INSERT INTO customers (first_name, last_name, address) VALUES ('McKevin', 'Aranda', 'Cavite');

SELECT * FROM vehicles;

SELECT * FROM policies; 

SELECT * FROM policy_holders JOIN policies ON policy_holders.pol_id = policies.id JOIN customers ON policies.customer_id = customers.id WHERE customers.id = 3;

SELECT * FROM policy_holders JOIN policies ON policy_holders.pol_id = policies.id JOIN customers ON policies.customer_id = customers.id WHERE customers.first_name = "lebron" AND customers.last_name = "james";

SELECT * FROM claims;

SELECT * FROM customers;

SELECT * FROM vehicles WHERE make = "q" AND model = "q" AND year = 2009 AND type = "q" AND fuel_type = "q" AND purchase_price = 823794 AND color = "q" AND premium_charged = 123456789;
